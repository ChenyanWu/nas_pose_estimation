...
class CE(nn.Module):
    def __init__(self, num_features, pooling=False, num_groups=1, num_channels=64, T=5, dim=4, eps=1e-5, momentum=0.1,
                 *args, **kwargs):
        ...
        self.eye = nn.Parameter(torch.eye(num_channels).expand(num_groups, num_channels, num_channels))
    ...
    def forward(self, X):
        ...
        if self.training:
            ...
            P = [None] * (self.T + 1)
            P[0] = torch.eye(d).to(X).expand(self.num_groups, d, d)
            # print(xc)
...
class CE_2(nn.Module):
    def __init__(self, channel, reduction=16):
        super(CE_2, self).__init__()
        self.reduction = reduction
        self.channelpool = ChannelAvgPool(reduction)
        assert channel % reduction == 0
        self.fc = nn.Sequential(
            nn.Linear(channel * channel // reduction // reduction, channel * channel // reduction // reduction,
                      bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(channel * channel // reduction // reduction, channel * channel // reduction // reduction,
                      bias=False),
    ...
    def forward(self, X):
        ...
        x_matix = torch.bmm(coef, X.view(N, C // self.reduction, -1)).view(N, C, H, W)
...
class ChannelMaxPool(nn.Module):
    def __init__(self, reduction=16):
        ...
        self.max_pool = nn.MaxPool1d(reduction)
        self.reduction = reduction
    def forward(self, input):
        ...
        # input = torch.nn.functional.max_pool1d(input, self.reduction)
        _, _, c = input.size()
...
class ChannelAvgPool(nn.Module):
    def __init__(self, reduction=16):
        ...
        self.avg_pool = nn.AvgPool1d(reduction)
        self.reduction = reduction
    def forward(self, input):
        n, c, h, w = input.size()
        input = input.view(n,c,h*w).permute(0,2,1).contiguous()
...
class COVARLayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.reduction = reduction
        self.channelpool = ChannelMaxPool(reduction)
        assert channel % reduction == 0
        self.fc = nn.Sequential(
            nn.Linear(channel * channel // reduction // reduction, channel // reduction, bias=False),
            nn.ReLU(inplace=True),
...
class COVARLayer_7(nn.Module):
    def __init__(self, channel, reduction=16):
        super(COVARLayer_7, self).__init__()
        self.reduction = reduction
        assert channel % reduction == 0
    ...
    def forward(self, x):
        ...
        x_weight = torch.sigmoid(self.x_weight)
        return x_weight * x_brach_avg + x_weight * x_branch_covar

class COVARLayer_8(nn.Module):
    def __init__(self, channel, reduction=16):
        super(COVARLayer_8, self).__init__()
        self.reduction = reduction
        assert channel % reduction == 0
...
class COVARLayer_11(nn.Module):
    def __init__(self, channel, reduction=16):
        super(COVARLayer_11, self).__init__()
        self.reduction = reduction
        assert channel % reduction == 0
    ...
    def forward(self, x):
        ...
        x_weight = torch.sigmoid(self.x_weight)
        # return nn.functional.relu((1-x_weight) * x_brach_avg + x_weight * x_instance + x)
        return (1 - x_weight) * x_brach_avg + x_weight * x_instance + x

class COVARLayer_12(nn.Module):
    def __init__(self, channel, reduction=16):
        super(COVARLayer_12, self).__init__()
        self.reduction = reduction
        assert channel % reduction == 0
...
class COVARLayer_13(nn.Module):
    def __init__(self, channel, reduction=16):
        super(COVARLayer_13, self).__init__()
        self.reduction = reduction
        assert channel % reduction == 0
...
class COVARLayer_15(nn.Module):
    def __init__(self, channel, reduction=16):
        super(COVARLayer_15, self).__init__()
        self.reduction = reduction
        assert channel % reduction == 0
...
class NasHighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
        ...
        self.covariance_layers = self._make_covariance_layers()
...
if __name__ == "__main__":
    ...
    args.cfg = "experiments/coco/covariance_hrnet/without_pretrain_w32_256x192_adam_lr1e-3.yaml"
