...
def train(config, train_queue_in_search, model, criterion, optimizer, optimizer_a, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_queue_in_search):
        ...
        if i % config.PRINT_FREQ == 1:
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
                  ...
                  'Accuracy {acc.val:.3f} ({acc.avg:.3f})'.format(
                      epoch, i, len(train_queue_in_search), batch_time=batch_time,
...
def val_plot(config, val_loader, val_dataset, model, criterion, output_dir,
             tb_log_dir, writer_dict=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
